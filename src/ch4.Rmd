...
ggplot(data = data.wide, mapping = aes(x = Post_1, y = Post_2, col = Sequence)) +
  facet_wrap(~Sequence) +
...
```{r subject-profiles}
ggplot(data = data.long %>% filter(Measurement == "Post"),
...
First we construct the ANOVA table, including the baseline by period interaction.
```{r anova}
...
summary(model)$coefficients %>% kbl()
```
