...
The ANOVA table \ref{anovaTable} can be easily generated using the built-in \mintinline{r}{aov()} and \mintinline{r}{summary()} functions, with the standard linear model notation as input.
\begin{minted}{R}
    # Table 3.1
...
\input{report/tables/estimatesTable.tex}
\input{report/tables/adjMeansTable.tex}
...
\subsubsection{In Practice}
As in section \ref{mixed-model-subsection}, first the data must be pivoted into long format, this time also with a column for within-subject baseline differences:
\begin{minted}{R}
...
\begin{minted}{R}
    ...
    data = data.baselines)
\end{minted}
At this point, the assumptions should be verified, as demonstrated in section \ref{verifying-assumptions-subsection}. The resulting estimates for the protein trial data are shown in INSERT TABLE, with the adjusted means shown in INSERT TABLE.
