...
This chapter uses data from Senn and Auclair \cite{senn1990graphical} as an example for the analysis presented. These data also consist of measurements of PEFR, this time on children suffering from athsma. The trial was a 2x2 cross-over design, comparing the effectiveness of two drugs; salbutamol (Sal) and formoterol (For). 13 patients were randomised into either a For-Sal sequence or a Sal-For sequence. The data are presented in table INSERT TABLE.

For analysis, cross-over data should usually be in 'long' format, as in table INSERT TABLE. All the analysis shown in the \textit{in practice} sections was done in the \textit{R} programming language, with the \mintinline{R}{tidyverse} package loaded. Any additional packages required are indicated where applicable.
...
Provided the data is in 'long' format, we can easily carry out a paired $t$-test using the \mintinline{R}{rstatix::t_test()} function. The result of the test on the PEFR data is outlined in \ref{tab:tTest}, showing a statistically significant difference between the For and Sal treatments.
